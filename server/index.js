...
app.post('/updatePizzasByPrice', function(request, response) 
{
    ...
    if (price!="not defined" && increment!="not defined" && lower!="not defined" && body!="body undefined")
	{
        console.log("okk");
		//aceptable input
		//search for a pizza
		var pizze = pizzaManager.updatePizzasByPrice(price, increment, lower);
		//if exists
		if (pizze != null)
		{
            console.log("not null");
			response.writeHead(200, headers);
			response.end(JSON.stringify(pizze));
		}
		else
		{
